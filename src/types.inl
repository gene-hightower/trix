...
// v177: inserting the is-set predicate into the SystemName enum shifts the
// binary-token / snapshot system-name indices of every later operator, so a
// pre-v177 image would resolve those indices to the wrong operator -- the
// version gate rejects pre-v177 images.
static constexpr uint32_t SNAPSHOT_VERSION{177};
public:
