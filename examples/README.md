...
A self-contained maze generator (~6400 lines), a homage to Steve Capps' "Amazing" on the 128K Mac.  Ships **eleven** maze algorithms (recursive-backtracker, Kruskal, Wilson, Eller, binary-tree, sidewinder, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree, and recursive-division -- the one wall-adding generator) across **five** grid topologies -- square (the full zoo), plus hex, theta polar, triangle, and upsilon octagon.  Seven of the eleven algorithms (backtracker, Kruskal, Wilson, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree) run on **every** grid through a shared topology descriptor; Eller/binary-tree/sidewinder/division stay square-only -- 35 portable combos.  Renders BFS distance-field heatmaps in ten colormaps with `--solve` / `--braid` / `--weave` overlays, and a `--metrics` quality report on any grid.  Output is a real PNG whose format is assembled in Trix (no libpng) over the engine's native `deflate` (zlib) and hand-rolled `crc32`/`adler32` ops.  Companion teaching + implementation guide: [amazing-manual.md](amazing-manual.md).  `--self-test` runs 153 assertions; [`gallery.sh`](gallery.sh) renders a 80-PNG showcase (including the full algorithm-by-topology matrix) into `examples/maze-gallery/` (gitignored).  Needs `--vm-size=8M`+ (more for `--stress`); see the manual for sizing.
