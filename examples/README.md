...
### [amazing.trx](amazing.trx) — 12-algo maze zoo + 7-grid topology + PNG output + bitmap font

A self-contained maze generator (~9,000 lines), a homage to Steve Capps' "Amazing" on the 128K Mac.  Ships **twelve** maze algorithms (recursive-backtracker, Kruskal, Wilson, Eller, binary-tree, sidewinder, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree, Origin Shift -- CaptainLuma's 2023 edge-reversal algorithm, every step a valid maze -- and recursive-division, the one wall-adding generator) across **seven** grid topologies -- square (the full zoo), plus hex, theta polar, triangle, upsilon octagon, zeta (square cells with diagonal passages -- each 2x2 quad pre-assigned one non-crossing diagonal), and crack (irregular Voronoi cells generated entirely in-engine by half-plane clipping + Lloyd relaxation, ~O(N) via spatial bucketing).  Eight of the twelve algorithms (backtracker, Kruskal, Wilson, Aldous-Broder, Prim, Hunt-and-Kill, Growing Tree, Origin Shift) run on **every** grid through a shared topology descriptor; Eller/binary-tree/sidewinder/division stay square-only -- 56 portable combos.  Renders BFS distance-field heatmaps in fourteen colormaps (including a vivid `fire-ice` diverging map and the cyclic `twilight`) with `--solve` / `--braid` / `--weave` overlays, a `--metrics` quality report on any grid, and a `--solver` zoo (bfs / dead-end-fill / A\* / Trémaux / wall-follower) that visualises *how* each method solves -- dead-end-fill drains the maze, A\* tints its focused frontier, Trémaux wanders the maze depth-first marking passages, wall-follower traces the left-hand walk.  Those metrics double as **targets**: `--target-dead-ends P` / `--target-loops N` braid the maze toward a requested dead-end percentage or exact loop count (monotone, connectivity-safe, best-effort report) on every grid including upsilon.  On big mazes where one linear sweep washes out, `--color-curve` (gamma) and `--color-cycles` (seamless sine-wave bands) reshape the ramp for local contrast.  `--unicursal` builds a single-path labyrinth (a doubled perfect maze) -- one non-branching corridor through every cell, with `--color` flowing the heatmap continuously along the one path.  `--flow radial|linear|spiral|sine` weights Kruskal by a scalar field for "flow mazes" whose corridors follow it (still perfect mazes on any grid; `--flow-jitter` dials art ↔ twisty), and `--flow-image NAME` steers that same flow by an image -- the bundled `logo` field traces the Trix wordmark and `cat` drapes the maze over a bold silhouette; sample your own PNG/SVG with `tools/gen_flow_field.py` (only the derived fields committed, see [`flow-fields/`](flow-fields/)).  **Masking** carves the maze into a shape -- a word (`--mask-text 'Amazing!'`), the real Trix logo cut out of a maze (`--mask logo`), any SVG (`--mask-file`), or an analytic disc/ring/frame -- with `--mask-invert` to punch the shape out of a full maze instead; disconnected shapes (letters) become one perfect maze per connected piece.  Text renders through a selectable `--font`: a built-in 5×7 block font, hi-res **Roboto** bitmap atlases (Apache-2.0), and **Hershey** stroke fonts (public domain) rendered in pure Trix -- generated from real faces by host tools (`tools/gen_mask_font.py` / `gen_hershey_font.py`), only the derived glyph data committed (see [`mask-fonts/`](mask-fonts/)).  The logo and any other SVG are rasterised to a 1-bit mask by `tools/gen_mask_svg.py` (only the logo's derived mask committed, see [`mask-shapes/`](mask-shapes/)).  The maze line and background colors are themeable (`--wall-color` / `--bg-color`, hex) on every topology.  Output is a real PNG whose format is assembled in Trix (no libpng) over the engine's native `deflate` (zlib) and hand-rolled `crc32`/`adler32` ops.  Companion teaching + implementation guide: [amazing-manual.md](amazing-manual.md).  `--self-test` runs 268 assertions; [`gallery.sh`](gallery.sh) renders a 133-PNG showcase (including the full algorithm-by-topology matrix) into `examples/maze-gallery/` (gitignored).  Needs `--vm-size=8M`+ (more for `--stress`); see the manual for sizing.
